  ...
  if(route.startsWith('#/sourcing')){
    ...
    addBtn.addEventListener('click',()=>{
      ...
      if(address&&price&&!isNaN(lat)&&!isNaN(lng)){
        ...
        state.data.properties.push({id,address,price,lat,lng,type:'',bedrooms:'',bathrooms:''});
        router();
      }
    });
    ...
    function selectProperty(id){
      ...
      if(marker){
        if(window.google?.maps){
          state.infoWin.setContent(`<div>${p.address}<br/>${p.price}<br/>${p.bedrooms||''} bd / ${p.bathrooms||''} ba<br/>${p.type||''}<br/><button id="addLead">Add to Leads</button></div>`);
    ...
    if(window.google?.maps){
      ...
      props.forEach(p=>{
        ...
        if(!isNaN(lat)&&!isNaN(lng)){
          const position=[lat,lng];
          const marker=L.marker(position).addTo(state.gmap).bindPopup(`<div>${p.address}<br/>${p.price}<br/>${p.bedrooms||''} bd / ${p.bathrooms||''} ba<br/>${p.type||''}<br/><button class='add-lead'>Add to Leads</button></div>`);
          state.markers[p.id]=marker;
          bounds.extend(position);
...
function parseCSV(text){
  ...
  return lines.map(line=>{
    ...
    const bedrooms=obj['Bedrooms']||'';
    const bathrooms=obj['Full Bathrooms']||'';
    return {id,address,price,lat,lng,type,bedrooms,bathrooms};
  });
}
